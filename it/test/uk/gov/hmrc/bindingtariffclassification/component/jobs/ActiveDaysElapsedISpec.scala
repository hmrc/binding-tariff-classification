...
class ActiveDaysElapsedJobISpec extends IntegrationSpecBase {
  ...
  private def givenThereIs(event: Event) = storeEvents(event)

  private def daysElapsedForCase: String => Long = { reference => getCase(reference).map(_.daysElapsed).getOrElse(0) }

  "ActiveDaysElapsed" when {

    "there are cases with mixed statuses in the database" should {
      "calculate elapsed days for OPEN & NEW cases" in {
    ...
    "there are cases with mixed statuses in the database" should {
      "calculates elapsed days for OPEN & NEW Liability cases" in {
    ...
    "a case which was REFERRED in the past" should {
      "calculate elapsed days for a referred case" in {
    ...
    "there is case which was REFERRED the day it was created" should {
      "calculates elapsed days for a case created & referred on the same day" in {
    ...
    "there is case with a referred case" should {
      "calculates elapsed days for a case referred today" in {
    ...
    "a Case which was SUSPENDED in the past" should {
      "calculates elapsed days for a suspended case" in {
    ...
    "there is case which was REFERRED the day it was created" should {
      "calculates elapsed days for a case created & suspended on the same day" in {
    ...
    "there is case with a referred case" should {
      "calculates elapsed days for a case suspended today" in {
    ...
    "there are migrated cases with mixed statuses in the database" should {
      "calculates elapsed days for migrated cases" in {
