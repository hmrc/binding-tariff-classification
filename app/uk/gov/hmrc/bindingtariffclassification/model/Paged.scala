...
case class Paged[T](results: Seq[T], pageIndex: Int = 1, pageSize: Int = 1, resultCount: Int = 1) {
  def map[X](f: T => X): Paged[X] = this.copy(results.map(f), pageIndex, resultCount)
