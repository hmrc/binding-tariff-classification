...
object DatabaseException {
  ...
  def exception(code: Int, message: String, bsonDocument: Option[BsonDocument] = None): MongoWriteException = {
    val writeError = new WriteError(code, message, bsonDocument.getOrElse(new BsonDocument()))
    val server     = ServerAddress("localhost")
    val label      = new util.ArrayList[String]()
    label.add("test-server-label")

    new MongoWriteException(writeError, server, label)
  }
