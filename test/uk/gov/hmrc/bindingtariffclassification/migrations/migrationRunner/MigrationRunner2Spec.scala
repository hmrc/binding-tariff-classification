...
class MigrationRunner2Spec extends BaseSpec {
  ...
  "MigrationRunner2" should {

    "not execute the job if the lock fails" in new TestSetup("MigrationJobTest2") {

      when(mockMigrationLockMongoRepository.findOne(any[String])).thenReturn(Future(None))
      when(mockMigrationLockMongoRepository.lock(any[JobRunEvent])).thenReturn(Future(false))
