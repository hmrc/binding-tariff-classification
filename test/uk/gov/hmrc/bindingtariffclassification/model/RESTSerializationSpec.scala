...
class RESTSerializationSpec extends BaseSpec {
  ...
  "Details" should {
    ...
    "Serialize and Deserialize JSON for NumberField" in {
      val field        = NumberField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[NumberField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for StatusField" in {
      val field        = StatusField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[StatusField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for LiabilityStatusField" in {
      val field        = LiabilityStatusField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[LiabilityStatusField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for CaseTypeField" in {
      val field        = CaseTypeField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[CaseTypeField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for ChapterField" in {
      val field        = ChapterField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[ChapterField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for DateField" in {
      val field        = DateField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[DateField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for StringField" in {
      val field        = StringField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[StringField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for DaysSinceField" in {
      val field        = DaysSinceField(fieldName = "context", underlyingField = "moreContext")
      val json         = Json.toJson(field)
      val deserialized = json.as[DaysSinceField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for StatusResultField" in {
      val field        = StatusResultField(fieldName = "context", data = Option(PseudoCaseStatus.NEW))
      val json         = Json.toJson(field)
      val deserialized = json.as[StatusResultField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for LiabilityStatusResultField" in {
      val field        = LiabilityStatusResultField(fieldName = "context", data = Option(LiabilityStatus.LIVE))
      val json         = Json.toJson(field)
      val deserialized = json.as[LiabilityStatusResultField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for CaseTypeResultField" in {
      val field        = CaseTypeResultField(fieldName = "context", data = Option(ApplicationType.BTI))
      val json         = Json.toJson(field)
      val deserialized = json.as[CaseTypeResultField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for DateResultField" in {
      val field        = DateResultField(fieldName = "context", data = Option(Instant.EPOCH))
      val json         = Json.toJson(field)
      val deserialized = json.as[DateResultField]

      deserialized.shouldBe(field)
    }
    "Serialize and Deserialize JSON for StringResultField" in {
      val field        = StringResultField(fieldName = "context", data = Option("moreContext"))
      val json         = Json.toJson(field)
    ...
    "Serialize and Deserialize JSON for AppealStatusChange" in {
      val appeal = AppealStatusChange(
        appealType = AppealType.APPEAL_TIER_2,
        from = AppealStatus.MEDIATION,
    ...
    "Serialize and Deserialize JSON for CaseResultGroup" in {
      val caseResultGroup = CaseResultGroup(
        count = 3L,
        groupKey = NonEmptySeq.one(NumberResultField("fieldName", Option(5L))),
        maxFields = List(NumberResultField("fieldName", Option(5L))),
        cases = List(btiCaseForFormatters)
      )
      val json         = Json.toJson(caseResultGroup)
      val deserialized = json.as[CaseResultGroup]

      deserialized.shouldBe(caseResultGroup)
    }

    "Serialize and Deserialize JSON for QueueResultGroup" in {
      val queue = QueueResultGroup(count = 2, team = Option("custodians"), caseType = ApplicationType.MISCELLANEOUS)
      val json  = Json.toJson(queue)
      val deserialized = json.as[QueueResultGroup]

      deserialized.shouldBe(queue)
    }

    "Serialize and Deserialize JSON for SimpleResultGroup" in {
      val group = SimpleResultGroup(
        count = 1L,
        groupKey = NonEmptySeq.one(NumberResultField("fieldName", Option(5L))),
        maxFields = List(NumberResultField("fieldName", Option(5L)))
      )
      val json         = Json.toJson(group)
      val deserialized = json.as[SimpleResultGroup]
