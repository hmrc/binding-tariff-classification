...
class MockAppConfig @Inject()(runModeConfiguration: Configuration, environment: Environment)
  extends AppConfig(runModeConfiguration, environment) {
  private val defaultSystemDate: Instant = LocalDate.parse("2019-02-01").atStartOfDay().toInstant(ZoneOffset.UTC)
